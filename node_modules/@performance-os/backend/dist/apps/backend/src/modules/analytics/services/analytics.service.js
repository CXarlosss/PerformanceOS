...
let AnalyticsService = class AnalyticsService {
    ...
    async getWeeklyAnalytics(userId) {
        ...
        const prevVolume = history.length > 0 ? this.volumeAggregator(history[0].workoutSessions) : 0;
    ...
    async getAthleteHistory(athleteId, range = '12w') {
        ...
        const sessions = await this.prisma.workoutSession.findMany({
            ...
            include: { sets: true, templateSession: true },
            orderBy: { date: 'asc' }
        });
        ...
        sortedWeeks.forEach((weekNum, index) => {
            ...
            const fatigue = weekSessions.reduce((total, s) => total + s.sets.reduce((st, set) => st + (set.load * set.reps * (set.rpe / 10)), 0), 0);
        ...
        sessions.forEach(s => {
            s.sets.forEach(set => {
                if (!exerciseProgress[set.exerciseName])
                    exerciseProgress[set.exerciseName] = [];
                const e1rm = set.load * (1 + set.reps / 30);
                exerciseProgress[set.exerciseName].push({
    ...
    volumeAggregator(sessions) {
        return sessions.reduce((total, s) => {
            return total + s.sets.reduce((st, set) => st + (set.load * set.reps), 0);
        }, 0);
    }
    getWeekNumber(date) {
