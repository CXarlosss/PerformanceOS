...
export class AnalyticsService {
  ...
  async getWeeklyAnalytics(userId: string) {
    ...
    if (!program) throw new NotFoundException("Athlete has no active program");
    ...
    const prevVolume =
      history.length > 0
        ...
        : 0;
    const progression = calculateVolumeProgression(currentVolume, prevVolume);
  ...
  async getAthleteHistory(athleteId: string, range: string = "12w") {
    const cutoffDate = new Date();
    if (range === "4w") cutoffDate.setDate(cutoffDate.getDate() - 28);
    else if (range === "8w") cutoffDate.setDate(cutoffDate.getDate() - 56);
    else if (range === "12w") cutoffDate.setDate(cutoffDate.getDate() - 84);
    ...
    sortedWeeks.forEach((weekNum, index) => {
      ...
      const volumeDelta =
        prevVolume > 0 ? ((volume - prevVolume) / prevVolume) * 100 : 0;
      const fatigueDelta =
        prevFatigue > 0 ? ((fatigue - prevFatigue) / prevFatigue) * 100 : 0;
      ...
      weeklyMetrics.push({
        ...
        fatigue,
        fatigueDelta,
        performanceScore: 70 + Math.random() * 20, // Placeholder mock
        prs,
    ...
    const trendAnalysis = {
      ...
      scoreTrend: this.calculateTrend(
        weeklyMetrics.map((m) => m.performanceScore),
      ),
  ...
  private detectStagnation(exerciseProgress: Record<string, any[]>): string[] {
    ...
    Object.keys(exerciseProgress).forEach((ex) => {
      ...
      if (last <= prev) stagnations.push(ex);
    });
    return stagnations;
  }
  ...
  private calculateTrend(values: number[]): string {
    ...
    if (diff > 5) return "ASCENDING";
    if (diff < -5) return "DESCENDING";
    return "PLATEAU";
  }
}
