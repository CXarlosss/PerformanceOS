...
export const useAnalytics = () => {
  ...
  const clientHistory = useMemo(
    () => (activeClientId ? trainingHistory[activeClientId] : []) || [],
    [trainingHistory, activeClientId],
  );
  ...
  const analytics = useMemo(() => {
    ...
    const volumeTrend = calculateVolumeProgression(
    ...
    const muscleVolumes = lastSession
      ? calculateVolumeByMuscleGroup(lastSession, EXERCISES)
      : {};
    ...
    const consistency = calculateConsistencyIndex(
      sessionsPlanned,
      activeMicrocycle.sessions.length,
    );
