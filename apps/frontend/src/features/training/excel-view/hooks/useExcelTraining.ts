...
export const useExcelTraining = () => {
  ...
  const currentSessionTemplate = useMemo(() => {
    ...
    return (
      activeMicrocycle.sessions.find((s: any) => s.id === selectedSessionId) ??
      null
    );
  }, [activeMicrocycle, selectedSessionId]);
  ...
  const currentWorkoutSession = useMemo(() => {
    ...
    return (
      program.workoutSessions.find(
        (ws: any) => ws.templateSessionId === selectedSessionId,
      ) ?? null
    );
  ...
  const tableData = useMemo(() => {
    ...
    return currentSessionTemplate.blocks.flatMap((block: any) =>
